...
class ProductProductRecordHandler(Component):
    ...
    def odoo_pre_write(self, odoo_record, values, orig_values):
        # When updating product.product records, product_tmpl_id
        # is set to False in values if not specified in the import type.
        # But we don't want to set it to False as the value
        # already exists in the odoo_record
        if (
