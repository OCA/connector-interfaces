...
# name of the models usable by file.buffer
available_tasks = ['repository.task']
def add_task(name):
...
class file_buffer(orm.Model):
    ...
    def _get_file_buffer_type(self, cr, uid, context=None):
        return self.get_file_buffer_type(cr, uid, context=context)

    def get_file_buffer_type(self, cr, uid, context=None):
        return []
    ...
    _columns = {
        ...
        'type': fields.selection(_get_file_buffer_type, 'Type'),
    }
    ...
    def create_file_buffer_attachment(self, cr, uid, file_buffer_id, datas,
        ...
        attach_obj = self.pool['ir.attachment']
        context.update({'default_res_id': file_buffer_id,
                        'default_res_model': 'file.buffer'})
    ...
    def run(self, cr, uid, ids, context=None):
        ...
        for filebuffer in self.browse(cr, uid, ids, context=context):
            self._run(cr, uid, filebuffer, context=context)
            if filebuffer.direction == 'input':
                filebuffer.done()
        return True

    def _run(self, cr, uid, filebuffer, context=None):
