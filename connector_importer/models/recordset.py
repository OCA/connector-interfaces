...
class ImportRecordset(models.Model):
    ...
    * schedule the import job for each import record
    ...
    def valid_source_api(self):
        context = dict(self.env.context)
        if self.source_model == "import.source.api":
            if not self.source_ref_id.token:
                message = _(
                    f"{self.source_ref_id.name}: "
                    f"The token value in the resource "
                    "cannot be empty."
                )
                if context.get("execute_import_manual", False):
