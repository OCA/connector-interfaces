...
class TestRecordImporter(TestImporterBase):
    ...
    def test_importer_check_missing_none(self):
        ...
        missing = importer._check_missing(
            'id', 'ref', values, orig_values)
    ...
    def test_importer_check_missing_source(self):
        ...
        missing = importer._check_missing(
            'fullname', 'name', values, orig_values)
        # name is missing now
        self.assertDictEqual(
            missing,
            {'message': 'MISSING REQUIRED SOURCE KEY=fullname: ref=doe'}
        )
        # drop ref
        ...
        self.assertDictEqual(
            missing,
            {'message': 'MISSING REQUIRED SOURCE KEY=id: ref=doe'}
        )
    ...
    def test_importer_check_missing_destination(self):
        ...
        self.assertDictEqual(
            missing,
            {'message': 'MISSING REQUIRED DESTINATION KEY=name: ref=doe'}
        )
        # drop ref
        values['name'] = name
        values.pop('ref')
        ...
        self.assertDictEqual(
            missing,
            {'message': 'MISSING REQUIRED DESTINATION KEY=ref'}
        )
