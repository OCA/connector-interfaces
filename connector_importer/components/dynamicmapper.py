...
class DynamicMapper(Component):
    ...
    `source_key_empty_skip` is used to skip keys when empty or no value is computed for them.
    ...
    `source_key_rename` is used to rename source keys to destination key (the real odoo field).
    `converter` is used to define custom converter options for specific fields.

        The value must be a dict containing the params to propagate to the converter function.
        Eg: for a m2o field `partner_id` that needs to be converted to a res.partner record
