...
class OdooRecordHandler(Component):
    ...
    # Inject context company
    _context_company = None
    ...
    def get_domain_company_context(self, values, orig_values):
        return []

    def _get_company_context(self, values, orig_values):
        company_id = self.env["res.company"].search_read(
            self.get_domain_company_context(values, orig_values), ["id"]
        )
        if len(company_id) > 0:
            self._context_company = company_id[0]["id"]

    def odoo_create(self, values, orig_values):
        """Create a new odoo record."""
        self.odoo_pre_create(values, orig_values)
        self._get_company_context(values, orig_values)
        # TODO: remove keys that are not model's fields
        odoo_record = (
            self.model.with_context(**self.create_context())
            .with_company(self._context_company)
            .create(values.copy())
        )
        # force uid
    ...
    def odoo_write(self, values, orig_values):
        ...
        odoo_record = (
            self.odoo_find(values, orig_values)
